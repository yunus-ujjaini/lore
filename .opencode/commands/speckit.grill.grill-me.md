...
<!-- Extension: grill -->
<!-- Config: .specify/extensions/grill/ -->
# SpecKit Grill Me
...
2. From the repository root, run `.specify/scripts/bash/check-prerequisites.sh --json --paths-only` **exactly once** to obtain `FEATURE_DIR` / `FEATURE_SPEC`. If it does not return JSON, stop and recommend rerunning the agent-specific invocation of `speckit.specify` or checking the active feature environment.
